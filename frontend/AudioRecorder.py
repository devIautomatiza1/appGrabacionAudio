...
class AudioRecorder:
    ...
    def rename_recording(self, old_filename: str, new_filename: str) -> bool:
        """
        Renombra un archivo de audio tanto localmente como en Supabase.
        ...
        try:
            # Validar que el nuevo nombre tenga extensión
            if '.' not in new_filename:
                ext = old_filename.split('.')[-1]
                new_filename = f"{new_filename}.{ext}"
            
            # Renombrar archivo local si existe
            old_path = RECORDINGS_DIR / old_filename
            new_path = RECORDINGS_DIR / new_filename
            
            if old_path.exists():
                old_path.rename(new_path)
                logger.info(f"Archivo local renombrado: {old_filename} -> {new_filename}")
            
            # Actualizar en Supabase Storage y BD
            sys.path.insert(0, str(Path(__file__).parent.parent))
            from backend.database import rename_recording_in_db, move_audio_in_storage
            
            # Primero mover en Storage
            storage_moved = move_audio_in_storage(old_filename, new_filename)
            
            # Luego actualizar en BD
            if rename_recording_in_db(old_filename, new_filename):
                logger.info(f"Renombrado en BD: {old_filename} -> {new_filename}")
                return True
            else:
                # Si falla en BD, revertir cambios
                if new_path and new_path.exists():
                    new_path.rename(old_path)
                if storage_moved:
                    # Intentar revertir en storage
                    move_audio_in_storage(new_filename, old_filename)
                logger.error(f"Error al renombrar en BD")
                return False
                
        except Exception as e:
            logger.error(f"Error al renombrar archivo: {e}")
            # Intentar revertir cambio local si existe
            if new_path and new_path.exists() and old_path and not old_path.exists():
                try:
                    new_path.rename(old_path)
                except:
                    pass
